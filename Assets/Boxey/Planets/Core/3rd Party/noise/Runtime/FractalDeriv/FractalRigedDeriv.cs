...
namespace Unbegames.Noise
{
    public struct FractalRigedDeriv<T> : INoise3D, INoiseDeriv3D where T : struct, INoiseDeriv3D
    {
        ...
        public FractalRigedDeriv(int octaves, float lacunarity = 1.99f, float gain = 0.5f, float weightedStrength = 0) :
            this(new T(), octaves, lacunarity, gain, weightedStrength)
        {
        }

        public FractalRigedDeriv(T noise, int octaves, float lacunarity = 1.99f, float gain = 0.5f,
        ...
        public real GetValue(int mSeed, real3 point, out real3 dsum)
        {
            ...
            {
                var noise = abs(mNoise.GetValue(seed++, point, out var deriv));
                dsum += deriv;
