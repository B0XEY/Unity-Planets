...
public class AtmosphereRenderPass : ScriptableRenderPass
{
    ...
    {
        if (!currentActiveEffects.Contains(effect)) currentActiveEffects.Add(effect);
    }
    ...
    private void CullAndSortEffects(Camera camera)
    {
        ...
        for (var i = currentActiveEffects.Count - 1; i >= 0; i--)
        {
            ...
            {
                var dstToSurface = currentActiveEffects[i].DistToAtmosphere(viewPos);
        ...
        for (var i = 0; i < visibleEffects.Count - 1; i++)
        for (var j = i + 1; j > 0; j--)
    ...
    {
        var cmd = CommandBufferPool.Get("Atmosphere Effects");
        cmd.Clear();
        ...
        var prefabMode = PrefabStageUtility.GetCurrentPrefabStage() == StageUtility.GetCurrentStage();
    ...
    private void RenderEffects(CommandBuffer cmd, RenderTargetIdentifier colorSource, bool inPrefabMode)
    {
        ...
        {
#if UNITY_EDITOR
            var stage = PrefabStageUtility.GetPrefabStage(visibleEffects[i].effect.gameObject);
            if (stage == null && inPrefabMode) continue;
#endif
            var blitMat = visibleEffects[i].effect.GetMaterial(atmosphereShader);
