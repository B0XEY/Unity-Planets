...
namespace Unbegames.Noise
{
    public struct FractalPingPongDeriv<T> : INoise3D, INoiseDeriv3D where T : struct, INoiseDeriv3D
    {
        ...
        public FractalPingPongDeriv(int octaves, float lacunarity = 1.99f, float gain = 0.5f,
            float weightedStrength = 0, float pingPongStength = 2) : this(new T(), octaves, lacunarity, gain,
            weightedStrength, pingPongStength)
        {
        }

        public FractalPingPongDeriv(T noise, int octaves, float lacunarity = 1.99f, float gain = 0.5f,
            float weightedStrength = 0, float pingPongStength = 2)
        {
            mNoise = noise;
        ...
        public real GetValue(int mSeed, real3 point, out real3 dsum)
        {
            ...
            {
                var noise = PingPong((mNoise.GetValue(seed++, point, out var deriv) + 1) * pingPongStength);
                dsum += deriv;
