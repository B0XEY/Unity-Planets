...
namespace Unbegames.Noise
{
    public struct FractalBillow<T> : INoise3D where T : struct, INoise3D
    {
        ...
        public FractalBillow(int octaves, float lacunarity = 1.99f, float gain = 0.5f, float weightedStrength = 0) :
            this(new T(), octaves, lacunarity, gain, weightedStrength)
        {
        }

        public FractalBillow(T noise, int octaves, float lacunarity = 1.99f, float gain = 0.5f,
        ...
        public real GetValue(int mSeed, real3 point)
        {
            ...
            {
                var noise = abs(mNoise.GetValue(seed, point)) * 2 - 1;
                sum += noise * amp;
