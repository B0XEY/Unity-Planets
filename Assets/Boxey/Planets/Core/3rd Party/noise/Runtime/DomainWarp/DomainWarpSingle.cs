...
namespace Unbegames.Noise
{
    public struct DomainWarpSingle<T, U> : INoise3D where T : struct, IDomainWarp3D where U : struct, INoise3D
    {
        ...
        public DomainWarpSingle(int warpSeed, float warpFrequency, float warpAmp) : this(new T(), new U(), warpSeed,
            warpFrequency, warpAmp)
        {
        }

        public DomainWarpSingle(U noise, int warpSeed, float warpFrequency, float warpAmp) : this(new T(), noise,
            warpSeed, warpFrequency, warpAmp)
        {
        }
