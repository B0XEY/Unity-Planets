...
namespace Unbegames.Noise
{
    public struct FractalPingPong<T> : INoise3D where T : struct, INoise3D
    {
        ...
        public FractalPingPong(int octaves, float lacunarity = 1.99f, float gain = 0.5f, float weightedStrength = 0,
            float pingPongStength = 2) : this(new T(), octaves, lacunarity, gain, weightedStrength, pingPongStength)
        {
        }

        public FractalPingPong(T noise, int octaves, float lacunarity = 1.99f, float gain = 0.5f,
        ...
        public real GetValue(int mSeed, real3 point)
        {
            ...
            {
                var noise = PingPong((mNoise.GetValue(seed++, point) + 1) * pingPongStength);
