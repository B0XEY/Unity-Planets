...
namespace Unbegames.Noise
{
    ...
    public struct OpenSimplex2 : INoise3D
    {
        public real GetValue(int seed, real3 p)
        {
            ...
            for (var l = 0;; l++)
            {
                if (a > 0) value += a * a * (a * a) * GradCoord(seed, ijk.x, ijk.y, ijk.z, p0.x, p0.y, p0.z);
                ...
                if (a0.x >= a0.y && a0.x >= a0.z)
                {
                    ...
                    {
                        b -= 1;
                        value += b * b * (b * b) * GradCoord(seed, ijk.x - nSign.x * PrimeX, ijk.y, ijk.z,
                            p0.x + nSign.x, p0.y, p0.z);
                    }
                }
                else if (a0.y > a0.x && a0.y >= a0.z)
                {
                    ...
                    {
                        b -= 1;
                        value += b * b * (b * b) * GradCoord(seed, ijk.x, ijk.y - nSign.y * PrimeY, ijk.z, p0.x,
                            p0.y + nSign.y, p0.z);
                    }
                }
                else
                {
                    ...
                    {
                        b -= 1;
                        value += b * b * (b * b) * GradCoord(seed, ijk.x, ijk.y, ijk.z - nSign.z * PrimeZ, p0.x, p0.y,
                            p0.z + nSign.z);
                    }
                }
