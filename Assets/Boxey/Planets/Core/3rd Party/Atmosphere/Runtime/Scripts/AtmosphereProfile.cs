...
[CreateAssetMenu(fileName = "NewAtmosphereProfile", menuName = "Boxey/Planet/Atmosphere Profile")]
public class AtmosphereProfile : ScriptableObject
{
    ...
    public ScatterWavelengths rayleighScatter = new() { red = 0.556f, green = 0.7f, blue = 0.84f, power = 2f };
    [Range(0, 100)] public float rayleighDensityFalloff = 15;


    public ScatterWavelengths mieScatter = new() { red = 1.0f, green = 0.95f, blue = 0.8f, power = 0.1f };
    [Range(0, 100)] public float mieDensityFalloff = 15;
    [Range(0, 1)] public float mieG = 0.97f;


    [Range(0, 100)] public float heightAbsorbtion;
    ...
    internal bool IsUpToDate(ref int LUTSize, ref int opticalPoints, ref float rayleighFalloff, ref float mieFalloff,
        ref float absorbtion)
    {
        var upToDate = (int)this.LUTSize == LUTSize &&
    ...
    internal void BakeOpticalDepth(ref RenderTexture opticalDepthTexture, ComputeShader shader, float planetRadius,
        float atmosphereRadius)
    {
        if (shader == null) throw new Exception("Compute Shader not provided");

        if (opticalDepthTexture == null || !opticalDepthTexture.IsCreated())
        {
            CreateRenderTexture(ref opticalDepthTexture, (int)LUTSize, (int)LUTSize, FilterMode.Bilinear,
            ...
            shader.GetKernelThreadGroupSizes(0, out var x, out var y, out _);

            var numGroupsX = Mathf.CeilToInt((int)LUTSize / (float)x);
            var numGroupsY = Mathf.CeilToInt((int)LUTSize / (float)y);
            shader.Dispatch(0, numGroupsX, numGroupsY, 1);
        }
    }


    private static void CreateRenderTexture(ref RenderTexture texture, int width, int height,
