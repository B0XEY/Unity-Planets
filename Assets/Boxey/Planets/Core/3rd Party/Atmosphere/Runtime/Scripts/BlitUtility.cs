...
{
    private static readonly int blitTargetA = Shader.PropertyToID("_BlitA");
    private static readonly int blitTargetB = Shader.PropertyToID("_BlitB");


    private static readonly RenderTargetIdentifier destinationA = new(blitTargetA);
    private static readonly RenderTargetIdentifier destinationB = new(blitTargetB);


    private static RenderTargetIdentifier latestDest;
    private static CommandBuffer blitCommandBuffer;
    ...
    public static void SetupBlitTargets(CommandBuffer cmd, RenderTextureDescriptor blitSourceDescriptor)
    {
        ...
        if (cmd == null) Debug.LogError("Blit Command Buffer is null, cannot set up blit targets.");

        var descriptor = blitSourceDescriptor;
