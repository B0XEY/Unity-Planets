...
namespace Unbegames.Noise
{
    public struct FractalFBM<T> : INoise3D where T : struct, INoise3D
    {
        ...
        public FractalFBM(int octaves, float lacunarity = 1.99f, float gain = 0.5f, float weightedStrength = 0) : this(
            new T(), octaves, lacunarity, gain, weightedStrength)
        {
        }
        ...
        public real GetValue(int mSeed, real3 point)
        {
            ...
            {
                var noise = mNoise.GetValue(seed++, point);
                sum += noise * amp;
